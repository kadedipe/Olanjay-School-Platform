...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="shell"><aside className="sidebar"><div className="brand"><Image className="brandLogo" src="/Olanjay_School_Logo.jpg" alt="Olanjay New Africa Technical School logo" width={1168} height={784} priority /><div><strong>Olanjay</strong><small>Technical School</small></div></div><nav aria-label="Main navigation"><Link href={`/dashboard/${user.role.toLowerCase()}`}>Overview</Link>{canViewStudents && <Link href="/dashboard/students">Students</Link>}<Link href="/dashboard/courses">Courses</Link><span className="navMuted">More modules unlock by role</span></nav><div className="profile"><div className="avatar">{initials}</div><div><strong>{user.name}</strong><small>{user.role.toLowerCase()}</small></div></div><SignOutButton /></aside>{children}</div>;
}
