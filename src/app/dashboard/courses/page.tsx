...
export default async function CoursesPage() {
  ...
  const courses = await prisma.course.findMany({ where: user.role === Role.ADMIN ? {} : { isActive: true }, include: { _count: { select: { enrollments: true, teachers: true } } }, orderBy: { code: "asc" } });
  return <main><header><div><p className="eyebrow">ACADEMIC CATALOGUE</p><h1>Courses</h1><p>{user.role === Role.ADMIN ? "Manage programmes, qualifications, and enrollment availability." : "Browse active programmes and their academic details."}</p></div>{user.role === Role.ADMIN && <Link className="primary buttonLink" href="/dashboard/courses/new">Add course</Link>}</header>
